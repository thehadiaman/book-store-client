...
class Cart extends Component {
    ...
    getTotalPrice = ()=>{
        let price = String(this.state.totalPrice);
        const sliced = _.slice(price, 0, price.length);

        price = '';
        for(let a=0;a<sliced.length;a++){
            if(a%3===0 && a!==(sliced.length-1)){
                price+=`${sliced[a]},`;
            }else{
                price+=sliced[a];
            }
                        ...
                        <Accordion>
                            ...
                            <AccordionDetails style={{padding: 0}}>
                                ...
                                <Grid container columns={2} padding={"10px"}>
                                    ...
                                    <Grid item xs={6}>
                                        <Button style={{width: "100%"}} variant={'contained'} color={'warning'} float={'right'}>Place Order</Button>
                                    </Grid>
                                </Grid>
                            </AccordionDetails>
                        </Accordion>
